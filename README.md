# Flutter Bài Tập Chương 4

**Sinh viên:** Đặng Ngọc Hiếu
**MSSV:** 6451071024
**Môn:** Lập Trình Ứng Dụng Trên Thiết Bị Di Động

---

## Cấu Trúc Thư Mục

```
lib/
├── main.dart
├── apps/
│   └── app.dart
├── models/
│   ├── dashboard_item.dart
│   ├── product.dart
│   └── product_item.dart
├── utils/
│   ├── util.dart
│   └── validate.dart
├── widget/
│   ├── alertdialog_custom.dart
│   ├── button_custom.dart
│   ├── dashboard_card.dart
│   ├── inputdecoration_custom.dart
│   └── product_card.dart
└── view/
    ├── dashboard/
    │   └── dashboard_screen.dart
    ├── 1/
    │   ├── home_view.dart
    │   └── about_view.dart
    ├── 2/
    │   ├── home_profile_view.dart
    │   └── profile_view.dart
    ├── 3/
    │   ├── home_setting.dart
    │   └── setting_view.dart
    ├── 4/
    │   ├── product_list_view.dart
    │   └── product_detail_view.dart
    ├── 5/
    │   ├── home_pick_color.dart
    │   └── pick_color_view.dart
    ├── 6/
    │   ├── home_stack.dart
    │   ├── page1_stack..dart
    │   └── page2_stack.dart
    ├── 7/
    ├── 8/
    └── 9/
```

---

## Danh Sách Bài Tập

### Bài 1: Home -> About (Named Route)
- Ảnh 1
![Bài 1](lib/view/1/image.png)
- Ảnh 2
![Bài 1](lib/view/1/image2.png)

---

### Bài 2: Home -> Profile

- Ảnh 1
![Bài 2](lib/view/2/image.png)
- Ảnh 2
![Bài 2](lib/view/2/image2.png)


---

### Bài 3: Home -> Setting (MaterialPageRoute)

- Ảnh 1
![Bài 3](lib/view/3/image.png)
- Ảnh 2
![Bài 3](lib/view/3/image2.png)

---

### Bài 4: Product List

- Ảnh 1
![Bài 4](lib/view/4/image.png)
- Ảnh 2
![Bài 4](lib/view/4/image2.png)

---

### Bài 5: Pick Color

- Ảnh 1
![Bài 5](lib/view/5/image.png)
- Ảnh 2
![Bài 5](lib/view/5/image2.png)

---

### Bài 6: Navigation Stack Management
- Ảnh 1
![Bài 6](lib/view/6/image.png)
- Ảnh 2
![Bài 6](lib/view/6/image2.png)
- Ảnh 3
![Bài 6](lib/view/6/image3.png)